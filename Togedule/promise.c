...
void selectDate(int CombineTimetable[5][13],char *DBname,structPromise *newPromise){	//만들 약속의 요일, 시간, 날짜 입력받는 역할을 하는 함수 (통합시간표, 학번+이름 문자열, 새약속구조체)를 인수로 받는다.
	...
	scanf("%s",time);
	printf("%s",dayofweek);				//계산한 요일을 통합시간표에 이용하기 위해 숫자로 바꿈
...
void promiseCreatConsole(char *DBname) {	//약속만들기 함수. 약속명, 인원수, 이름검색(함수호출)을 여기서 한다. (학번+이름)을 인수로 받는다.
	char Name[13];
	int i,j;
	int Count=0;
	int CountCopy;
