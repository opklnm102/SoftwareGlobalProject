...
int searchName(char *name,int count,struct structPromise newPromise,int CombineTimetable[5][13],char *MyDB,char IDList[4][13]){		//이름검색함수.. (검색할이름, 함께하는회원중 리스트에 안올라간 회원수,새약속구조체,통합시간표,'학번+이름'문자열,현재 저장된 회원리스트) 를 변수로 받는다.
	...
	while(1){		
		...
		while(1) {						//리스트 번호 범위 내의 수만 입력받기
			...
			gotoxy(x,34);printf("리스트 범위 내의 숫자를 입력하세요.");
			gotoxy(x+20,33);printf("     ");
			strcpy(select,"\0");
		}
		limit=1;
...
int callendar(int Month,char *dateLimit) {		//달력출력함수. 월을 인수로 넘겨받는다. selectDate 함수에서 입력받은 달을 넘겨받음.
...
int selectDate(int CombineTimetable[5][13],structPromise *newPromise){	//만들 약속의 날짜 입력받는 함수
	...
	while(1){
		...
		if(!strcmp(month,"1월")||!strcmp(month,"2월")||!strcmp(month,"3월")||!strcmp(month,"4월")||!strcmp(month,"5월")||!strcmp(month,"6월")||!strcmp(month,"7월")||!strcmp(month,"8월")||!strcmp(month,"9월")||!strcmp(month,"10월")||!strcmp(month,"11월")||!strcmp(month,"12월")) {
			gotoxy(x,y+5); printf("                                            ");
			break;
		}
		else if(!strcmp(month,"b")||!strcmp(month,"B"))
			return -1;
		else if(!strcmp(day,"x")||!strcmp(day,"X")){
				gotoxy(90,45);
				exit(0);
		}
		gotoxy(x+30,y+4); printf("                             ");
		gotoxy(x,y+5); printf("약속을 잡을 달은 1월~12월 사이로 입력하세요.");
	}
	...
	year=callendar(Month,dateLimit);							//입력받은 월을 가지고 callendar함수사용. 올해 달력을 출력 
	
	datelimit=atoi(dateLimit);
	gotoxy(x,y+15);printf("요일은 날짜입력에 따라 자동으로 선택됩니다.");
	gotoxy(x,y+16);printf("날짜입력이 끝나면 약속시간을 입력하세요.");
	gotoxy(x,y+17);printf("교시숫자에 8을 더하면 간편하게 시간을 계산할 수 있습니다.");
	while(1){
		gotoxy(x,y+19);printf("약속을 잡을 날짜 입력(ex 10일) :");		//일을 입력하면
		while(1){
			while(1){
				gotoxy(x+34,y+19);scanf("%s",&day);
				if(!strcmp(day,"1일")||!strcmp(day,"2일")||!strcmp(day,"3일")||!strcmp(day,"4일")||!strcmp(day,"5일")||!strcmp(day,"6일")||!strcmp(day,"7일")||!strcmp(day,"8일")||!strcmp(day,"9일")||!strcmp(day,"10일")) {
				
					gotoxy(x,y+20); printf("                                                       ");
					break;
				}
				else if(!strcmp(day,"11일")||!strcmp(day,"12일")||!strcmp(day,"13일")||!strcmp(day,"14일")||!strcmp(day,"15일")||!strcmp(day,"16일")||!strcmp(day,"17일")||!strcmp(day,"18일")||!strcmp(day,"19일")||!strcmp(day,"20일")) {
					gotoxy(x,y+20); printf("                                                       ");
					break;
				}
				else if(!strcmp(day,"21일")||!strcmp(day,"22일")||!strcmp(day,"23일")||!strcmp(day,"24일")||!strcmp(day,"25일")||!strcmp(day,"26일")||!strcmp(day,"27일")||!strcmp(day,"28일")||!strcmp(day,"29일")||!strcmp(day,"30일")||!strcmp(day,"31일")) {
					gotoxy(x,y+20); printf("                                                       ");
					break;
				}
				else if(!strcmp(day,"b")||!strcmp(day,"B"))
					return -1;
				else if(!strcmp(day,"x")||!strcmp(day,"X")){
				gotoxy(90,45);
				exit(0);
				}
				gotoxy(x+34,y+19);printf("             ");
				gotoxy(x,y+20);printf("약속일은 달력의 범위 내에서 숫자+일 단위로 입력하세요.");
				strcpy(day,"\0");
			}
			...
			gotoxy(x,y+20); printf("                                                       ");
			gotoxy(x,y+20);printf("%d월달은 %d일까지만 입력 가능한 달입니다.",Month,datelimit);
			gotoxy(x+34,y+19);printf("             ");
		}
		...
		gotoxy(x,y+20);printf("약속은 주중에만 잡을 수 있습니다.");
		gotoxy(x+34,y+19);printf("      ");
		strcpy(day,"\0");
	}
	return dayofWeek;
...
void selectTime(int CombineTimetable[5][13],structPromise *newPromise,int dayofWeek){
	...
	gotoxy(x,y);printf("약속시간을 입력 (ex 13시) :");
	gotoxy(x,y+1);printf("ex) 5교시에 약속을 잡으려면 13시");
	while(1){
		gotoxy(x+27,y);scanf("%s",&time);
		if(!strcmp(time,"x")||!strcmp(time,"X")){
			gotoxy(90,45);
			exit(0);
		}
		else if(!strcmp(time,"9시")||!strcmp(time,"10시")||!strcmp(time,"11시")||!strcmp(time,"12시")||!strcmp(time,"13시")||!strcmp(time,"14시")||!strcmp(time,"15시")||!strcmp(time,"16시")||!strcmp(time,"17시")||!strcmp(time,"18시")||!strcmp(time,"19시")||!strcmp(time,"20시")||!strcmp(time,"21시")) {
			gotoxy(x,y+2); printf("                                                            ");
			break;
		}
		else if(!strcmp(time,"b")||!strcmp(time,"B"))
			return;	
		gotoxy(x,y+2);printf("약속시간은 9시~21시 사이의 정각 시간을 입력하세요.");
	...
	for(i=0;i<length;i++) {
		...
		if(strncmp(time,"시",1)==0) {				//'시'라는 단어를 제외하고 숫자부분만 떼내어서 정수형 변수 Time에 저장  
			Time=atoi(temp);
			break;
		}		
	}	
	...
	while(1){						//시간이 통합시간표에 겹치는 시간이면 재입력. 제대로 입력시 넘김
		...
		if(errorCheck==0)
			break;
		gotoxy(x+27,y);printf("        ");
		gotoxy(x,y+3);printf("해당 시간은 비어있지 않습니다."); 
		gotoxy(x,y+4);printf("통합 시간표를 참고해서 선택한 날짜의 빈시간을 입력하세요");
...
void selectName(char *DBname,struct structPromise *newPromise) {	//약속명 함수. (학번+이름)을 인수로 받는다.
	...
	if(!strcmp(newPromise->promiseName,"0")) {}
	else if(!strcmp(newPromise->promiseName,"x")||!strcmp(newPromise->promiseName,"X")){
			gotoxy(90,45);
			exit(0);
		}
	else if(stringLengthcheck(newPromise->promiseName,40)) {
		gotoxy(x+10,y-1);printf("약속명은 한글 20자, 영문 40자 이하로 입력하세요.");
		gotoxy(x+10,y);printf("                                         ");
		gotoxy(x+50,y); printf(" 인원수 :                                           ");}  //문자열의 길이 체크
				
		else{  //오류 없을시		    
			gotoxy(x+10,y-1); printf("                                                "); //오류메시지 라인클리어
			break;  
		}
	}
...
int selectFriends(char *DBname,int CombineTimetable[5][13],struct structPromise *newPromise) {	//약속만들기 함수. 약속명, 인원수, 이름검색(함수호출)을 여기서 한다. (학번+이름)을 인수로 받는다.
	...
	while(1){
		...
		if(!strcmp(newPromise->promiseFreindsCount,"b")||!strcmp(newPromise->promiseFreindsCount,"B"))
			return 0;
		else if(!strcmp(newPromise->promiseFreindsCount,"x")||!strcmp(newPromise->promiseFreindsCount,"X")){
			gotoxy(90,45);
			exit(0);
			}
		else if(!strcmp(newPromise->promiseFreindsCount,"1")||!strcmp(newPromise->promiseFreindsCount,"2")||!strcmp(newPromise->promiseFreindsCount,"3")||!strcmp(newPromise->promiseFreindsCount,"4")){  //오류 없을시		    
			gotoxy(x+50,y-1); printf("                                                "); //오류메시지 라인클리어
			break;  
		}
		gotoxy(x+50,y-1);printf("인원수는 1~4 범위의 숫자로 입력하세요.");
		gotoxy(x+50,y); printf(" 인원수 :                                           ");
	}
	...
	while(Count>0) {		//인원수만큼 검색함수를 돌린다.
		...
		while(1){
			...
			if(!strcmp(Name,"b")||!strcmp(Name,"B"))			
				break;
			else if(!strcmp(Name,"x")||!strcmp(Name,"X")){
			gotoxy(90,45);
			exit(0);
			}
			else if(stringLengthcheck(Name,12)) {
					gotoxy(x+12,y+3);printf("                                            ");
				gotoxy(x+12,y+3);printf("이름은 한글 6자내로 입력하세요.");
				gotoxy(x+12,y+4);printf("                                        ");
			}
			else if(hanErrorcheck(Name)) {
					gotoxy(x+12,y+3);printf("                                            ");
				gotoxy(x+12,y+3);printf("이름은 한글로 입력하세요.");
				gotoxy(x+12,y+4);printf("                                        ");
			}
			else {
				gotoxy(x+12,y+3);printf("                                            ");
