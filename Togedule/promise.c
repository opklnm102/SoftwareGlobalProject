...
int searchName(char *name,int count,struct structPromise newPromise,int CombineTimetable[5][13]){		//이름 검색함수. txt파일의 중복이름은 김진삼.txt 김진삼1.txt 김진삼2.txt 이런형태로 가정해서 풀어나감.
	...
	while(1){		//이름중복이 있는지 확인하고 있으면 파일을 열어 이름을 가져오는 반복문 (바깥 루프)
		...
		fp = fopen(openDB, "r");
		if(!fp){
			printf("%s 회원의 시간표가 없습니다.",name);//이름을 검색한 회원의 시간표는 없어도 계속 진행가능해야함...지금 오류나면서 터짐
		}
		while (!feof(fp)) {
...
void promiseCreatConsole(char *DBname) {	//약속만들기 함수 (약속명, 인원수 입력  .이름 검색함수 내부 포함. 약속 리스트 출력)
	...
	printf(" 인원수 : ");
	scanf("%s",&newPromise.promiseFreindsCount);// 리스트 출력할때는 다름
	...
	printf("\n 약속멤버");			//리스트 출력부분
			//Count를 위에서 0으로 만들었으므로 다시 int형으로 재배치.
	printf("\n%s",nameList);
...
void promise(structMember *s){
	...
	strcpy(logID,s->ID);
	strcpy(logName,s->name);//이름을 기반으로 txt파일을 찾는 형식. 이름을 logID에 복사해준다.
	strcpy(DBname,logID);
	strcat(DBname,logName);
	showMenu();

	promiseList(logID,logName);				//현재회원 약속리스트 출력
