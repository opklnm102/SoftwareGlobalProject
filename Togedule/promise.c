...
int selectDate(int CombineTimetable[5][13],structPromise *newPromise){	//만들 약속의 날짜 입력받는 함수
	...
	while(1){
		...
		while(1){
			...
			gotoxy(x,y+19);printf("약속일은 달력의 범위 내에서 숫자+일 단위로 입력하세요.");
			strcpy(day,"\0");
		}
...
void selectTime(int CombineTimetable[5][13],structPromise *newPromise,int dayofWeek){
	...
	gotoxy(x,y+1);printf("ex) 1교시에 약속을 잡으려면 9시,5교시에 약속을 잡으려면 13시");
	while(1){
		gotoxy(x+27,y);scanf("%s",&time);
		if(!strcmp(time,"9시")||!strcmp(time,"10시")||!strcmp(time,"11시")||!strcmp(time,"12시")||!strcmp(time,"13시")||!strcmp(time,"14시")||!strcmp(time,"15시")||!strcmp(time,"16시")||!strcmp(time,"17시")||!strcmp(time,"18시")||!strcmp(time,"19시")||!strcmp(time,"20시")||!strcmp(time,"21시")) {
			gotoxy(x,y+2); printf("                                                            ");
			break;
		}
		else if(!strcmp(time,"b")||!strcmp(time,"B"))
			return;	
		gotoxy(x,y+2);printf("약속시간은 9시~21시 사이의 정각 시간을 입력하세요. ex) 13시");
...
int selectFriends(char *DBname,int CombineTimetable[5][13],struct structPromise *newPromise) {	//약속만들기 함수. 약속명, 인원수, 이름검색(함수호출)을 여기서 한다. (학번+이름)을 인수로 받는다.
	...
	while(1){
		...
		if(!strcmp(newPromise->promiseFreindsCount,"1")||!strcmp(newPromise->promiseFreindsCount,"2")||!strcmp(newPromise->promiseFreindsCount,"3")||!strcmp(newPromise->promiseFreindsCount,"4")){  //오류 없을시		    
