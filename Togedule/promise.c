...
int callendar(int Month) {		//달력출력함수. 월을 인수로 넘겨받는다. selectDate 함수에서 입력받은 달을 넘겨받음.
	...
	gotoxy(26,17);printf(" 일 월 화 수 목 금 토");		//입력받은 년도와 월의 달력을 출력만 하는 함수이다.
	gotoxy(26,18);printf(" --------------------");
	date=1;
	for(x=0;x<=6;x++)
	{	
...
int selectDate(int CombineTimetable[5][13],structPromise *newPromise){	//만들 약속의 날짜 입력받는 함수
	...
	gotoxy(x,y+1); printf("달력이 나타나면 통합시간표를 참고해서 약속을 정할 날짜를 입력하세요.");


	gotoxy(x,y+3); printf("약속을 잡을 달 입력(ex 3월) :");	
	gotoxy(x+30,y+3); scanf("%s",&month);								//월을 입력하면
	length=strlen(month);
